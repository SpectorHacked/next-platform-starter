...
export default function Header() {
    ...
    return (
        <>
            ...
            <div className="md:hidden sticky top-0 z-50 w-full bg-blue-600">
                <div className="px-4">
                    <div className="flex h-16 items-center justify-end">
                        ...
                        >
                            <span className={`block w-5 h-0.5 bg-white transition-transform duration-200 ${isMobileMenuOpen ? 'rotate-45 translate-y-1.5' : ''}`}></span>
                            <span className={`block w-5 h-0.5 bg-white transition-opacity duration-200 ${isMobileMenuOpen ? 'opacity-0' : ''}`}></span>
                            <span className={`block w-5 h-0.5 bg-white transition-transform duration-200 ${isMobileMenuOpen ? '-rotate-45 -translate-y-1.5' : ''}`}></span>
                        </button>
                    </div>
                </div>
            ...
            {/* Mobile Sidebar */}
            <div className={`fixed top-0 right-0 z-[70] h-full w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out md:hidden ${
